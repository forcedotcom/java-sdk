...
|logout-from-sfdc|This attribute controls whether a logout from the OAuth application also logs the user out of Database.com. This logout is accomplished by redirecting the user to the Database.com logout page so when it is set to true the default-logout-success URL will be ignored. The default value is false|
